...
$.ajax({
	url: "http://localhost:8000/get_brands_similarity/",
	type: "POST",
	data: {
		'pic': GET["pic"],
	},
	...
	success: function (result) {
		...
		g.selectAll(".brand")
			...
			.attr("fill", function (d) {
				return "rgba(255, 0, 0, " + alpha(d.sim) + ")";
			})
			.attr("stroke", "#578290");
...
d3.json("http://localhost:8000/es_tool/json/caption.json").then(function (data) {
	...
	var dataset = data['captions'].filter(function (clusters) {
		return clusters.cluster === target;
	})[0];
	...
	var score = d3.scaleLinear()
		.domain([d3.min(dataset.children, function (d) {
			return d.score;
	...
	node.append("circle")
		...
		.attr("fill", function (d) {
			return "rgba(255, 0, 0, " + alpha(d.data.score) + ")";
		})
		.attr("stroke", "#578290");
	...
	node.append("text")
		...
		.text(function (d) {
			...
			return Math.floor(score(d.data.score));
		})
		.attr("font-family", "Karla")
		.attr("font-size", function (d) {
			return d.r / 3;
	...
	node.append("title")
		.text(function (d) {
			return d.data.topic + ": " + score(d.data.score);
		});
});
...
d3.json("http://localhost:8000/es_tool/json/tags.json").then(function (result) {
	...
	var target = GET['target'];
	var data = result['tags'].filter(function (clusters) {
		return clusters.cluster === target;
	})[0].children;
	...
	tags.selectAll(".tag")
		...
		.style("background-color", function (d) {
			return "rgba(255, 140, 18, " + alpha(d.score) + ")";
		});
});
